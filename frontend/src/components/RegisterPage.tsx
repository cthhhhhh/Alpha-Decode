...
interface RegisterPageProps {
  onRegisterSuccess: (token: string, role: string, username: string, level?: number, xp?: number, maxUnlockedLessonIndex?: number, streak?: number) => void;
  onGoToLogin: () => void;
  onBack: () => void;
}
...
export default function RegisterPage({ onRegisterSuccess, onGoToLogin, onBack }: RegisterPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onRegisterSuccess(data.token, data.role, data.username, data.level, data.xp, data.maxUnlockedLessonIndex, data.streak);
    } catch (err) {
